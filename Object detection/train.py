...
def train_oe(model, data_loader, optimizer, scheduler, combined_loss, epochs=10, device='cpu'):
    """
    As above, except using a combined_loss which takes (y_pred, y_true, t_pred, t_true) as input.
    """
    # Move model to the specified device
    model = model.to(device)
    ...
    for epoch in range(epochs):
        ...
        for X_batch, Y_batch, gt_bbox, t_batch in tqdm(data_loader, desc=f"Epoch {epoch+1}/{epochs}"):
            ...
            total_loss = combined_loss(class_score.squeeze(), Y_batch, t_vals, t_batch)
        ...
        # Print the epoch's total loss (slight error in averaging here)
        print(f"Epoch [{epoch+1}/{epochs}], Loss: {epoch_loss / len(data_loader)}")
